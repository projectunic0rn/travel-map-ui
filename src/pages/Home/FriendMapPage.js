...
const GET_ALL_USER_COUNTRIES = gql`
  query {
    users {
      id
      username
...
const FriendMapPage = () => {
  ...
  function handleLoadedCountries(data) {
    let countryArray = clickedCountryArray;
    let allFriendData = data.users;
    allFriendData.map(friendData => {
      if (friendData.Places_visited.length != 0) {
        for (let i = 0; i < friendData.Places_visited.length; i++) {
          countryArray.push({
            countryId: friendData.Places_visited[i].country,
            tripTiming: 0
          });
        }
      }
      if (friendData.Places_visiting.length != 0) {
        for (let i = 0; i < friendData.Places_visiting.length; i++) {
          countryArray.push({
            countryId: friendData.Places_visiting[i].country,
            tripTiming: 1
          });
        }
      }
      if (friendData.Place_living !== null) {
        countryArray.push({
          countryId: friendData.Place_living.country,
          tripTiming: 2
        });
      }
    });
